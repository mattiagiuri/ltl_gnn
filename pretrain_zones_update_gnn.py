...
def main():
    ...
    for seed in seeds:
        command = [
            ...
            '--epochs', '10',
            '--num_steps', '2_000_000',
            '--model_config', 'pretraining_gnn_PointLtl2-v0',
            '--curriculum', 'pretraining_update_PointLtl2-v0',
            '--name', args.name,
            '--seed', str(seed),
            '--device', args.device,
...
if __name__ == '__main__':
    if len(sys.argv) == 1:  # if no arguments are provided, use the following defaults
        sys.argv += '--num_procs 16 --device cpu --name test_gnn_10 --seed 5 --log_csv false --save true'.split(' ')
    try:
        main()
