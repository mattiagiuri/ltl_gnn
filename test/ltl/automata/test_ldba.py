...
def test_complete_sink_state1():
    ...
    assert_transitions_equal(ldba, 1, {
        LDBATransition(1, 1, 'a', True),
        LDBATransition(1, 2, '!a', False)
    })
    assert_transitions_equal(ldba, 2, {
        LDBATransition(2, 2, 't', False)
    })
...
def test_complete_sink_state2():
    ...
    assert len(ldba.state_to_transitions) == 3
    assert_transitions_equal(ldba, 0, {
        LDBATransition(0, 0, 'a & !b', False),
        LDBATransition(0, 1, 'b & !a', False),
        LDBATransition(0, 2, '(a & b) | (!a & !b)', False)
    })
    assert_transitions_equal(ldba, 1, {
        LDBATransition(1, 1, 'a', True),
        LDBATransition(1, 2, '!a', False)
...
def test_complete_sink_state3():
    ...
    assert_transitions_equal(ldba, 1, {
        ...
        LDBATransition(1, 3, '!b', False)
    })
    assert_transitions_equal(ldba, 2, {
        LDBATransition(2, 2, 'a', True),
        LDBATransition(2, 3, '!a', False)
    })
    assert_transitions_equal(ldba, 3, {
        LDBATransition(3, 3, 't', False)
    })
