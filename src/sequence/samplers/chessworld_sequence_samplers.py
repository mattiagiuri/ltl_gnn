...
def chessworld_sample_reach(depth: int | tuple[int, int]) -> Callable:
...
def chessworld_sample_reach_avoid(
...
# def chessworld_sample_reach_stay(num_stay: int, num_avoid: tuple[int, int]) -> Callable[[list[str]], LDBASequence]:
#     def wrapper(propositions: list[str]) -> LDBASequence:
#         reach = random.choice(all_assignments)
