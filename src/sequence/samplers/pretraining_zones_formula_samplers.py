...
def pretraining_zonenv_sample_reach(depth: int | tuple[int, int]) -> Callable:
...
def pretraining_zonenv_sample_reach_avoid(
    ...
    def wrapper(propositions: list[str]) -> LDBASequence:
        ...
        def sample_one_color(last_reach, cur_d):
            ...
            if mode == 'and':
                available_reach = [k for k, a in all_ands_dict.items() if
                                   (not last_reach.issubset(a)) ] if not_reach_same_as_last else [k for k, a in
                                                                                                  all_ands_dict.items()
                                                                                                  ]
                ...
                available_reach = [k for k, a in complete_var_assignments.items() if
                                   (not last_reach.issubset(a))] if not_reach_same_as_last else [
                    k for k, a in complete_var_assignments.items()]
            ...
            if na > 0:
                available_avoid = [k for k, a in colors_only.items() if
                                   (not last_reach.issubset(a)) or len(last_reach) == 0]
        ...
        def sample_one_area(last_reach, last_ra_encoding):
            ...
            if na == 0:
                ...
                all_avoid_keys = ['right', 'top', 'left', 'bottom',
                                  'bottom&left', 'bottom&right',
                                  'left&top', 'right&top']

                possible_avoids = [ak for ak in all_avoid_keys if (not last_reach.issubset(retrieve_areas(ak))) or len(last_reach) == 0]

                avoid_keys_areas = [random.choice(possible_avoids)]
...
if __name__ == '__main__':
    ...
    sample_info_dict = {'blue': {Quadrant.TOP_RIGHT, Quadrant.TOP_LEFT}, 'green': {Quadrant.BOTTOM_LEFT},
                        'magenta': {Quadrant.BOTTOM_RIGHT, Quadrant.TOP_LEFT},
                        'yellow': {Quadrant.BOTTOM_LEFT, Quadrant.TOP_LEFT}}
