...
for num_ands in range(1, 3):
    ...
    if num_ands == 1:
        cur_formula_data = organized_formulae["and"]["positive"][2]
    else:
    ...
    for and_set in cur_formula_data:
        ...
        for num_avoid in range(1, 3):
            if num_avoid == 1:
                possible_sets = organized_formulae["or_x_and_not_y"]["positive"][(num_avoid, num_ands + 1)]
            else:
                possible_sets = organized_formulae["or_x_and_not_ny"]["positive"][(num_avoid, 2, num_ands)]
...
def chessworld8_sample_simple_reach(depth: int | tuple[int, int], num_ors: int | tuple[int, int]) -> Callable:
...
def chessworld8_sample_complex_reach(depth: int | tuple[int, int]) -> Callable:
    def wrapper(propositions: list[str]) -> LDBASequence:
        d = random.randint(*depth) if isinstance(depth, tuple) else depth
...
def chessworld8_sample_formulae_reach_avoid(
...
def chessworld8_sample_formula_reach_stay(num_stay: int, num_avoid: tuple[int, int]) -> Callable[[list[str]], LDBASequence]:
    def wrapper(propositions: list[str]) -> LDBASequence:
        available = []
        c = 0
