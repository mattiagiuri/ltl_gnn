...
print("Reach")
Xsr, edgesr, rootsr = tree.process_sequence(sample_reach, lens,False)
print("Avoid")
Xsa, edgesa, rootsa = tree.process_sequence(sample_avoid, lens, False)
...
print(Xsr)
print([tree.embedding_dict[i] for i in Xsr])

print()
print("Features avoid")
print(Xsa)
print([tree.embedding_dict[i] for i in Xsa])
...
embedding_reach = sample_embedding(torch.tensor(Xsr, dtype=torch.long))
embedding_avoid = sample_embedding(torch.tensor(Xsa, dtype=torch.long))
