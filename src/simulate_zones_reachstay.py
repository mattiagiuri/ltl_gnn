...
print(len(reach_any))
reach_green = frozenset([Assignment.single_proposition('green', props).to_frozen()])
avoid_none = frozenset()
avoid_others = frozenset([
    Assignment.zero_propositions(props).to_frozen(),
    *[Assignment.single_proposition(p, props).to_frozen() for p in props if p != 'green']
])
seq = [(reach_green, avoid_none), *[(reach_green, avoid_others)] * 1]  # TODO: get step
